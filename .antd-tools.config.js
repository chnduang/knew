...
// We need compile additional content for knew user
function finalizeCompile() {
  if (fs.existsSync(path.join(__dirname, './lib'))) {
    // Build a entry less file to dist/knew.less
...
function buildThemeFile(theme, vars) {
  // Build less entry file: dist/knew.${theme}.less
  if (theme !== 'default') {
    fs.writeFileSync(
      path.join(process.cwd(), 'dist', `knew.${theme}.less`),
      `@import "../lib/style/${theme}.less";\n@import "../lib/style/components.less";`,
    );
    // eslint-disable-next-line no-console
    console.log(`Built a entry less file to dist/knew.${theme}.less`);
  } else {
    fs.writeFileSync(
...
function finalizeDist() {
  if (fs.existsSync(path.join(__dirname, './dist'))) {
    // Build less entry file: dist/knew.less
    fs.writeFileSync(
      path.join(process.cwd(), 'dist', 'knew.less'),
    ...
    // eslint-disable-next-line no-console
    console.log('Built a entry less file to dist/knew.less');
