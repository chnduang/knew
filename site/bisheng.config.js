...
module.exports = {
  ...
  webpackConfig(config) {
    config.resolve.alias = {
      'knew/lib': path.join(process.cwd(), 'components'),
      'knew/es': path.join(process.cwd(), 'components'),
      // Change antd from `index.js` to `site/antd.js` to remove deps of root style
      knew: path.join(process.cwd(), 'site', 'knew'),
    ...
    config.plugins.push(
      new webpack.DefinePlugin({
        antdReproduceVersion: JSON.stringify(antBaseVersion),
      }),
    );
