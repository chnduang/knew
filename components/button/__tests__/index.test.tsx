...
describe('Button', () => {
  ...
  it('should warning when pass a string as icon props', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      `Warning: [knew: Button] \`icon\` is using ReactNode instead of string naming in v4. Please check \`search\` at https://ant.design/components/icon`,
    );
    warnSpy.mockRestore();
  });
  ...
  it('should warning when pass type=link and ghost=true', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      "Warning: [knew: Button] `link` or `text` button can't be a `ghost` button.",
    );
    warnSpy.mockRestore();
  });
